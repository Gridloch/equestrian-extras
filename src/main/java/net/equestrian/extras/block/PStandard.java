...
public class PStandard extends Block implements Waterloggable {
    ...
    public PStandard(Settings settings) {
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (blockView.getBlockState(blockPos.down()).isOf(this)) {
            if (blockView.getBlockState(blockPos.up()).isOf(this)) {
                return this.getDefaultState().with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER).with(Properties.HORIZONTAL_FACING, ctx.getPlayerFacing().getOpposite()).with(UP, 2).with(DOWN, 2);
            }
            else if (blockView.getBlockState(blockPos.up()).isIn(EquestrianExtras.BlockTags.STANDARDS)) {
                return this.getDefaultState().with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER).with(Properties.HORIZONTAL_FACING, ctx.getPlayerFacing().getOpposite()).with(UP, 1).with(DOWN, 2);
            }
            else
                return this.getDefaultState().with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER).with(Properties.HORIZONTAL_FACING, ctx.getPlayerFacing().getOpposite()).with(UP, 0).with(DOWN, 2);
        }
        else if (blockView.getBlockState(blockPos.down()).isIn(EquestrianExtras.BlockTags.STANDARDS)) {
            if (blockView.getBlockState(blockPos.up()).isOf(this)) {
                return this.getDefaultState().with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER).with(Properties.HORIZONTAL_FACING, ctx.getPlayerFacing().getOpposite()).with(UP, 2).with(DOWN, 1);
            }
            else if (blockView.getBlockState(blockPos.up()).isIn(EquestrianExtras.BlockTags.STANDARDS)) {
            ...
            if (blockView.getBlockState(blockPos.up()).isOf(this)) {
                return this.getDefaultState().with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER).with(Properties.HORIZONTAL_FACING, ctx.getPlayerFacing().getOpposite()).with(UP, 2).with(DOWN, 0);
            }
            else if (blockView.getBlockState(blockPos.up()).isIn(EquestrianExtras.BlockTags.STANDARDS)) {
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        ...
        if (neighborState.isIn(EquestrianExtras.BlockTags.PANELSTANDARDS) && direction.getAxis().getType() == Direction.Type.VERTICAL) {
