...
public class SaddleRack extends Block {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        BlockState blockState2 = this.getDefaultState().with(ON_WALL, false).with(SADDLE, false).with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing().getOpposite());
        Direction direction = ctx.getSide();
        if (direction == Direction.DOWN || direction != Direction.UP && ctx.getHitPos().y - (double)blockPos.getY() > 0.1) {
            return blockState2.with(ON_WALL, true).with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing().getOpposite());
        }
        return blockState2;
    }
